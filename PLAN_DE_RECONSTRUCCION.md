...
6. ~~Una pantalla de configuración de cuenta con cambio de contraseña y el flujo de "olvidé mi contraseña" (Errores 2.A.1 y 12.5).~~ **HECHO.** "Configuración" está en el menú lateral de los ocho perfiles (`/configuracion`) y muestra los datos leídos de la base. El cambio de contraseña exige la actual y cierra las sesiones de los demás dispositivos. "Olvidé mi contraseña" y "¿Problemas para ingresar?" (que eran un `console.log` y un `href="#"`) ahora llevan a `/recuperar-contrasena` y `/ayuda-de-acceso`. Vive en `servidor/cuenta.js` (`/api/cuenta` y `/api/recuperacion`), `NEXO/src/servicios/cuenta.ts` y la tabla nueva `codigos_recuperacion`. **Con una salvedad grande:** NEXO todavía no manda correos, así que el código de un solo uso **se imprime en la terminal del servidor**; hoy solo puede recuperar su contraseña quien vea esa terminal. Cuando haya envío de correos se cambia la función `entregarCodigo` de `servidor/cuenta.js` y nada más.
